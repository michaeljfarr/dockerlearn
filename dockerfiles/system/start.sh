...
read_compose_spec()
{
	...
	container_name="$(grep -s "name:" $containerFile | awk '{print $2}')"
	container_tag="$(grep -s "name:" $containerFile | awk '{print $3}')"
	container_ports="$(grep -s "ports:" $containerFile | awk '{print $2}')"
	local container_working_dir="$(grep -s "working_dir:" $containerFile | awk '{print $2}')"
	...
	persistent_volume_name="$(grep -s "persistent_volume: " $containerFile | awk '{print $2}')"
	...
	do
		awkFetch="'{""print $""$a""}'"
		#echo $awkFetch
		local volume_spec="$(grep -s "persistent_volume $persistent_volume_name" $containerFile | eval "awk $awkFetch")"
		if [ -n "$volume_spec" ] 
			then
	...
	do
		awkFetch="'{""print $""$a""}'"
		#echo $awkFetch
		local link_spec="$(grep -s "links: " $containerFile | eval "awk $awkFetch")"
		if [ -n "$link_spec" ] 
			then
...
connect_container()
{
	...
	echo "docker run -t -i -p $container_ports -v /app:/app $persistent_volume_spec $link_specs $container_working_dir_spec $container_tag /bin/bash"
	echo "expected run command: $run_container in $container_working_dir_spec"

	docker run -t -i -p $container_ports -v /app:/app $persistent_volume_spec $link_specs $container_working_dir_spec  $container_tag /bin/bash
}
...
run_container()
{
	...
	if  !(is_container_running $container_name) ; then
		...
		echo "docker run -td -p $container_ports -v /app:/app $persistent_volume_spec $link_specs $container_working_dir_spec --name=$container_name $container_tag /bin/bash $run_container"
		docker run -td -p $container_ports -v /app:/app $persistent_volume_spec $link_specs $container_working_dir_spec --name=$container_name $container_tag /bin/bash $run_container
		sleep 7s
		if  !(is_container_running $container_name) ; then
...
start_container()
{
	...
	if  is_container_running $container_name ; then
		...
		echo "docker run -p $container_ports -v /app:/app $persistent_volume_spec $container_tag /bin/bash $init_container"
		docker run -p $container_ports -v /app:/app $persistent_volume_spec $container_tag /bin/bash $init_container
...
if [ -d $containerDir ] && [ -n $containerDir ] ; then
	echo "$containerDir found doing $2 ..."
	eval "$2 $1"
else
